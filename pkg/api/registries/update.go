...
func Update(store storage.Store, logger log.Logger) http.HandlerFunc {
	logger = log.WithPrefix(logger, "registries", "update")
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := store.UpdateRegistry(record); err != nil {
			level.Warn(logger).Log(
				"msg", "failed to update registry",
				"err", err,
			)

			fail.Error(w, fail.Cause(err).BadRequest("failed to update registry"))
			return
		}
