...
// Update updates the current profile.
func Update(store storage.Store, logger log.Logger) http.HandlerFunc {
	logger = log.WithPrefix(logger, "profile", "update")

	return func(w http.ResponseWriter, r *http.Request) {
		record := session.Current(r.Context())

		if err := jsoniter.NewDecoder(io.LimitReader(r.Body, session.CurrentBodyLimit)).Decode(record); err != nil {
		...
		if err := store.UpdateUser(record); err != nil {
			...
			fail.Error(w, fail.Cause(err).BadRequest("failed to update profile"))
			return
		}
